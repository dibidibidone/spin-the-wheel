...
export function GatesScene({ config }: { config?: LandingSceneConfig } = {}) {
  ...
  if (!webgl) {
    return (
      <div className={status === "nearmiss" ? "shake" : status === "won" ? "boom" : undefined} style={{ position: "fixed", inset: 0, background: "radial-gradient(120% 90% at 50% 0%, #2a1f5c 0%, #120c2e 70%)" }}>
        {reels}
        {overlay}
  ...
  return (
    <div className={status === "nearmiss" ? "shake" : status === "won" ? "boom" : undefined} style={{ position: "fixed", inset: 0, background: "#120c2e" }}>
